...
var CONSUMER_KEY_PROPERTY_NAME = "22CVMM";
// Key of ScriptProperty for Fitbit consumer secret.
var CONSUMER_SECRET_PROPERTY_NAME = "910c202203d7a764f23c1ad28baa9042";
...
var LOGGABLES = ["activities/tracker/steps", "activities/log/distance",
...
function sync() {
    ...
    var colIndex = 1
    var dateColDone = 0;
    var File = SpreadsheetApp.openById("1k-P-EQKojmN8Q4KwZ9momidx2GzBlTF7co7-NnQLimo");
    SpreadsheetApp.setActiveSpreadsheet(File);
    
    var doc = File.getActiveSheet()
    var doc = File.getSheetByName("Fitbit2")
    doc.setFrozenRows(4);
    ...
    for (var activity in activities) {
        ...
        try {
            var result = UrlFetchApp.fetch("https://api.fitbit.com/1/user/-/"
                                           + currentActivity  + //"/date/" + getFirstDate() + "/" + dateString +
                                           ".json" + "?beforeDate=today&sort=desc&limit=100&offset=0", options);
        } catch (exception) {
            Logger.log(exception);
        ...
        var fitbitData = JSON.parse(result.getContentText());
        // set title
        ...
        for (var i in fitbitData) {
            ...
            var rowData = fitbitData[i]; //pagination
            for (var j = rowData.length - 1; j >= 0; j--) {
                var vals = rowData[j];
                
                
                cell.offset(indexRow, 0).setValue(vals["lastModified"].split("T")); // first column of data
                
                cell.offset(indexRow, 1).setValue(vals["activityName"]); // second column of data
                
                var distance = vals["distance"];
                Logger.log(distance);
                cell.offset(indexRow, 2).setValue(distance);
                
                if (distance != undefined) {
                    cell.offset(indexRow, 3).setValue(vals["distance"]/vals["speed"]);  ///60/1000/60);
                } else {
                    cell.offset(indexRow, 3).setValue(0);
                }
                
                cell.offset(indexRow, 4).setValue(vals["steps"]);
